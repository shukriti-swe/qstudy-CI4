...
class CorporateController extends BaseController
{
    ...
    {   
        ...
        return view('corporate/corporate_dashboard',$data);
    }
    ...
    public function corporate_setting()
    {
		...
		$data['user_info']=$SchoolClass->getInfo('tbl_useraccount','id',$this->session->get('user_id'));
		$data['page_title'] = '.:: Q-Study :: Tutor yourself...';       
        return view('corporate/corporate_setting', $data);
    }

    public function corporate_info_details(){
        $SchoolClass = new \SchoolClass();
		$data['user_info']=$SchoolClass->userInfo($this->session->get('user_id'));

		$data['corporate_info']=$SchoolClass->getTutorInfo($this->session->get('user_id'));
		//echo '<pre>';print_r($data['tutor_info']);die;
		$data['page_title'] = '.:: Q-Study :: Tutor yourself...';       
		return view('corporate/corporate_details',$data);
	}
    ...
    public function corporate_logo()
    {
		...
		$data['user_info']=$SchoolClass->getInfo('tbl_useraccount', 'id', $this->session->get('user_id'));
		$data['page_title'] = '.:: Q-Study :: Tutor yourself...'; 
        return view('corporate/upload',$data);
    }
    ...
    public function corporate_logo_upload()
	{
        ...
        if ($profile_image != '') {
            ...
            if($image)
            {
                $corporate_profile_picture=$profile_images_name ;
                $data = array(
                    'image' =>$corporate_profile_picture
                );
